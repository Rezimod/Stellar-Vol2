      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn-primary py-3 rounded-xl text-sm font-bold mt-2"
          >
            {loading ? t('auth.loggingIn') : t('auth.login')}
          </button>
        </form>

        <div className="flex justify-between mt-6 text-xs" style={{ color: 'var(--text-dim)' }}>
          <Link href="/signup" className="hover:text-[#38F0FF] transition-colors">
            {t('auth.noAccount')}{' '}
            <span style={{ color: '#38F0FF' }}>{t('nav.signup')}</span>
          </Link>
          <button className="hover:text-[#38F0FF] transition-colors">
